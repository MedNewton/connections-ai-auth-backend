...
const FORBIDDEN_RTDB_CHAR_CODES = new Set<number>([
  46,  // .
  35,  // #
  36,  // $
  91,  // [
...
function isValidRtdbKeyCharCode(cc: number): boolean {
  return !FORBIDDEN_RTDB_CHAR_CODES.has(cc);
}

function sanitizeRtdbKey(raw: string): string {
  const s = raw.trim();
  let out = "";
...
export function userKeyFromUser(user: {
  ...
  if (id) return `u_${sanitizeRtdbKey(id)}`;

  const email = typeof user.email === "string" ? user.email.trim() : "";
  if (email) return `e_${sanitizeRtdbKey(email.toLowerCase())}`;
